...
from .runner import RunnerError, run_hunyuan_i2v
from .store import JobRecord, JobStore

app = FastAPI(title="HunyuanVideo-I2V Pod API", version="0.1.0")
store = JobStore()
...
def _load_params(params_json: str | None) -> InferenceParams:
    if not params_json:
        return InferenceParams(
            ...
            infer_steps=settings.default_infer_steps,
            video_length=settings.default_video_length,
            embedded_cfg_scale=settings.default_embedded_cfg_scale,
        )
...
def _run_job(job_id: str, params: InferenceParams) -> None:
    ...
    try:
        result = run_hunyuan_i2v(record.image_path, output_path, params)
...
async def create_job(
    ...
    record = JobRecord(job_id=job_id, prompt=prompt, image_path=image_path)
    store.create(record)
